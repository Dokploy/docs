...
2. In your Git repository settings, paste this URL into the Webhook URL field.
3. Make sure to select content type `application/json`.
4. Choose the events that should trigger deployments, such as `Push`.
5. Once configured, any specified action, like pushing a new commit, will automatically trigger a deployment.

Dokploy supports webhooks from GitHub, GitLab, Bitbucket, Gitea.
